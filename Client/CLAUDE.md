...
### 1차 출시 형태: Station 20분 웨이브 서바이벌

로비에서 게임 시작 시 Station 씬으로 진입. 지하철 이동 없이 단일 맵에서 20분 동안 웨이브를 막아낸다.
...
[로비] -> 게임 시작 -> [Station 씬]
  ...
  - 웨이브 적 스폰 시작
  - 2분마다 Elite 몬스터 등장
  - Elite 처치 시 변이 아이템 드롭 -> 상호작용으로 변이 획득
  - 20분 생존 -> GameClear
  - 전멸 -> GameOver
```
...
`GameManager._survivalMode = true` (기본값)이면 지하철 루프를 건너뛰고 서바이벌 모드로 진행.
`false`로 바꾸면 기존 Subway-Station 반복 루프로 복귀 (향후 로그라이크 요소 추가 시 사용).
