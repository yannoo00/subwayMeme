...
### 핵심 게임 루프
```
[지하철 탑승] → [지하철 내 전투] → 적 전멸? ─── YES ──→ [역 도착] → [역 파밍/회복] → [재탑승] → 반복
                                       │                                                      ↓
                                       NO                                             문 닫히기 전 탑승!
                                       │
                                       ↓
...
- **타이머 종료 시 분기**:
  - 적 전멸 → 역 도착 (파밍/회복 가능)
  - 적 생존 → 역 스킵, 랜덤 다음 역으로 자동 이동. 이전 적 유지 + 새 웨이브 추가 스폰
- **역**: 항상 적이 없는 안전 지대 (적 있을 때는 절대 역에 도착하지 않음)
- **재탑승**: 역 대기 타이머 종료 전에 지하철 탑승
- **놓침 페널티**: 탑승 실패 시 처리 방식 미정
...
│   ↓ 출발 시 적 스폰 (이전 생존 적 + 새 웨이브)         │
│   ↓ n분 경과 (타이머)                                 │
│                                                      │
│   적 전멸? ─── YES ──→ [역 도착] (MapType: Station)   │
│       │                    ↓ 역에는 항상 적 없음        │
│       NO                   ↓ 파밍/회복/상점            │
│       │                    ↓ 역 대기 타이머             │
│       ↓                    ├─ [탑승 성공] → 반복       │
...
- **진행 순서**:
  ...
  **3-2. 적 스폰**
  - 출발과 동시에 적 스폰 시작
  - 역 스킵으로 이어지는 구간이면 이전 생존 적 유지 + 새 WaveData 추가 스폰
  - 첫 출발이면 현재 floor의 WaveData로만 스폰
  - 스폰 위치: 지하철 내부 Spawn Points

  **3-3. 전투 진행**
  - 플레이어는 지하철 내에서 적과 전투
  - 적을 모두 처치해야만 역에 내릴 수 있음

  **3-4. 타이머 종료 시 분기**
  - 지하철 출발 후 n분(예: 3분) 경과 시 분기 판정
  - **적 전멸 상태**: Station 씬 로드 → 역 도착
  - **적 생존 상태**: 역 스킵 → 랜덤 다음 노드 선택 → 이전 적 유지 + 새 웨이브 추가 스폰 → 타이머 재시작

#### 4. 역 도착 (휴식/상호작용 구간)
- **상태**: `GameState.Station`
- **맵 타입**: `MapType.Station`
- **씬**: `Station_Normal.scene`
- **도달 조건**: 지하철 타이머 종료 시 적이 0명일 때만 도달. 역에는 항상 적이 없음.
- **진행 순서**:

  **4-1. 역 도착**
  - Station 씬 로드
  - `StageEvents.OnStationArrived` 발생
  - 역 내 모든 상호작용 요소 즉시 활성화 (자판기, 상점, 회복 아이템 등)
  - 하차구(SubwayExit)에서 E키로 자유롭게 하차 가능
  ...
  **4-3. 재출발 타이머**
  ...
  - 출발 10초 전 경고: "곧 출발합니다!"
  - 타이머 종료 시 → 지하철 문 닫힘 + 탑승 판정

#### 5. 지하철 재탑승 판정
- **5-1. 탑승 성공**
  - 플레이어가 출발 시간 전에 지하철 내부에 있는 경우
  - 노선도 UI 열림 → 다음 역 선택 → 출발
  - 새로운 적 스폰 (다음 구간 WaveData)
  - → 다음 지하철 구간으로 진행

- **5-2. 탑승 실패 (놓침 페널티)**
  - 플레이어가 출발 시간까지 지하철 내부에 없는 경우
  - 처리 방식 미정 (역에 적이 침입하는 방식은 삭제됨)
...
- SpawnManager는 `_aliveEnemies` 리스트에 계속 보관
- Station 씬 로드 없이 Subway 씬 유지
- 다음 floor의 새 WaveData 추가 스폰 → 두 그룹이 함께 존재

#### 지하철 → 역 전환 시 (타이머 종료, 적 전멸):
- 적이 0명이므로 Station 씬 로드
- SpawnManager `_aliveEnemies` 리스트는 이미 비어있음

#### 역 → 지하철 전환 시 (재탑승):
- 역에는 적이 없으므로 이전 적 없음
- 새로운 WaveData 기반으로만 스폰

### 상호작용 요소 (역 내부)

역에는 항상 적이 없으므로 모든 상호작용 요소는 **항상 활성화**.
